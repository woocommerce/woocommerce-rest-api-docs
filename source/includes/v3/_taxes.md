...
```shell
curl -X PUT https://example.com/wc-api/v3/taxes/bulk \
	-u consumer_key:consumer_secret \
	-H "Content-Type: application/json" \
	-d '{
  "taxes": [
...
WooCommerce.put('taxes/bulk', data, function(err, data, res) {
  console.log(res);
});
```
...
print(wcapi.put("taxes/bulk", data).json())
```
...
woocommerce.put("taxes/bulk", data).parsed_response
```
